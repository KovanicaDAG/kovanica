...
pub struct AssetRegistryEntry {
    ...
    /// Whether the asset is fungible or non-fungible.
    pub kind: AssetKind,
    /// Maximum supply (must be 1 for NonFungible).
    pub max_supply: u64,
...
impl AssetRegistryEntry {
    ...
    /// Whether this asset is fully minted (minted == max_supply).
    pub fn is_fully_minted(&self) -> bool {
        self.minted >= self.max_supply
    }

    /// Whether this is an NFT (kind == NonFungible).
    pub fn is_nft(&self) -> bool {
        self.kind == AssetKind::NonFungible
    }
}
...
/// Derives a deterministic RWA (KVP-106) asset_id from issuer key and asset parameters.
///
/// This follows the RFC-007 specification:
/// asset_id = SHA256("KVP106-RWA" || issuer_pubkey || asset_class || unique_id || version)
///
/// # Arguments
/// * `issuer` - Ed25519 public key of the issuer (32 bytes)
/// * `asset_class` - Asset class string (e.g., "RE", "BOND", "INVOICE")
/// * `unique_id` - Issuer-defined unique identifier for this asset
/// * `version` - Version byte (currently 1)
///
/// # Returns
/// * `AssetId` - 32-byte deterministic identifier
...
mod tests {
    ...
    #[test]
    fn rwa_asset_id_derivation_deterministic() {
        let issuer = [0x42u8; 32];
        let asset_id1 = derive_rwa_asset_id(&issuer, "RE", "tower-12a", 1);
        let asset_id2 = derive_rwa_asset_id(&issuer, "RE", "tower-12a", 1);
        assert_eq!(asset_id1, asset_id2);
    }

    #[test]
    fn rwa_asset_id_different_params() {
        let issuer = [0x42u8; 32];
        let asset_re = derive_rwa_asset_id(&issuer, "RE", "tower-12a", 1);
        let asset_bond = derive_rwa_asset_id(&issuer, "BOND", "tower-12a", 1);
        let asset_invoice = derive_rwa_asset_id(&issuer, "INVOICE", "tower-12a", 1);
        let asset_diff_id = derive_rwa_asset_id(&issuer, "RE", "tower-12b", 1);
        let asset_diff_version = derive_rwa_asset_id(&issuer, "RE", "tower-12a", 2);
        let asset_diff_issuer = derive_rwa_asset_id(&[0x43u8; 32], "RE", "tower-12a", 1);

        // All should be different
        assert_ne!(asset_re, asset_bond);
        assert_ne!(asset_re, asset_invoice);
        assert_ne!(asset_re, asset_diff_id);
        assert_ne!(asset_re, asset_diff_version);
        assert_ne!(asset_re, asset_diff_issuer);
        assert_ne!(asset_bond, asset_invoice);
        assert_ne!(asset_bond, asset_diff_id);
        assert_ne!(asset_bond, asset_diff_version);
        assert_ne!(asset_bond, asset_diff_issuer);
        assert_ne!(asset_invoice, asset_diff_id);
        assert_ne!(asset_invoice, asset_diff_version);
        assert_ne!(asset_invoice, asset_diff_issuer);
        assert_ne!(asset_diff_id, asset_diff_version);
        assert_ne!(asset_diff_id, asset_diff_issuer);
        assert_ne!(asset_diff_version, asset_diff_issuer);
    }

    #[test]
    fn rwa_asset_id_not_native() {
        let issuer = [0x42u8; 32];

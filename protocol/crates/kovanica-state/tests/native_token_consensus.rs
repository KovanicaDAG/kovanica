...
// =========================================================================
// Category: KVP-106 NFT (Non-Fungible Assets) Tests
// =========================================================================
...
fn test_nft_mint_via_coinbase() {
    ...
    // Mint NFT via coinbase with value = 1
    let coinbase = Transaction::coinbase(
    ...
    let res = apply_block(&mut utxo, &[coinbase], 1_000);
    assert!(res.is_ok());
    assert_eq!(utxo.balance_of_asset(&alice.address(), Some(nft_asset)), 1);
}

#[test]
fn test_nft_transfer() {
    let alice = generate_key(51);
    let bob = generate_key(52);
    let nft_asset = make_asset_id(101);
    ...
    let spend = Transaction::signed(
        &[(coin, &alice)],
        vec![TxOutput::new(1, Some(nft_asset), bob.address())],
        b"nft_transfer".to_vec(),
    );

    let res = apply_block(&mut utxo, &[spend], 0);
    assert!(res.is_ok());
    assert_eq!(utxo.balance_of_asset(&bob.address(), Some(nft_asset)), 1);
    assert_eq!(utxo.balance_of_asset(&alice.address(), Some(nft_asset)), 0);
}

#[test]
fn test_nft_value_must_be_one() {
    let alice = generate_key(53);
    let bob = generate_key(54);
    let nft_asset = make_asset_id(102);
    ...
    // Try to transfer with value != 1 (should fail)
    let spend = Transaction::signed(
        &[(coin, &alice)],
        vec![TxOutput::new(2, Some(nft_asset), bob.address())], // value = 2
        b"nft_bad_value".to_vec(),
    );
...
#[test]
fn test_nft_cannot_split() {
    let alice = generate_key(55);
    let bob = generate_key(56);
    let charlie = generate_key(57);
    let nft_asset = make_asset_id(103);
    ...
    // Try to split NFT into two outputs (should fail)
    let spend = Transaction::signed(
        &[(coin, &alice)],
        vec![
            TxOutput::new(1, Some(nft_asset), bob.address()),
            TxOutput::new(1, Some(nft_asset), charlie.address()),
        ],
        b"nft_split".to_vec(),
    );
...
#[test]
fn test_nft_double_mint_rejected() {
    use kovanica_state::Ledger;

    let alice = generate_key(58);
    let bob = generate_key(59);
    let nft_asset = make_asset_id(104);
    ...
    // First mint
    let coinbase1 = Transaction::coinbase(
        vec![TxOutput::new(1, Some(nft_asset), alice.address())],
        b"nft_mint_1".to_vec(),
    );
    ledger
...
#[test]
fn test_nft_mint_then_transfer_then_receive() {
    let alice = generate_key(60);
    let bob = generate_key(61);
    let charlie = generate_key(62);
    let nft_asset = make_asset_id(105);
    ...
    // Alice -> Bob
    let spend1 = Transaction::signed(
        &[(coin, &alice)],
        vec![TxOutput::new(1, Some(nft_asset), bob.address())],
        b"alice_to_bob".to_vec(),
    );
    let coin2 = OutPoint::new(spend1.id(), 0);
    apply_block(&mut utxo, &[spend1], 0).unwrap();
    assert_eq!(utxo.balance_of_asset(&bob.address(), Some(nft_asset)), 1);

    // Bob -> Charlie
    let spend2 = Transaction::signed(
        &[(coin2, &bob)],
        vec![TxOutput::new(1, Some(nft_asset), charlie.address())],
...
#[test]
fn test_nft_asset_kind_registry() {
    ...
    assert_eq!(entry.minted, 0);
    assert_eq!(entry.metadata_hash, Some([0x11u8; 32]));
    assert_eq!(entry.collection_id, Some([0x22u8; 32]));
    assert_eq!(entry.creator, Some([0x33u8; 32]));
    assert!(entry.is_nft());
    assert!(!entry.is_fully_minted());
}

#[test]
fn test_fungible_asset_kind_registry() {
    use kovanica_state::{AssetKind, AssetRegistryEntry};

    let asset_id = make_asset_id(201);
    let entry = AssetRegistryEntry::new_fungible(asset_id, 1_000_000);

    assert_eq!(entry.kind, AssetKind::Fungible);
    assert_eq!(entry.max_supply, 1_000_000);
    assert_eq!(entry.minted, 0);
    assert!(!entry.is_nft());
    assert!(!entry.is_fully_minted());
}

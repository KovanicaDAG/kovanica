            ...
            for (block, stored_id) in iter {
                ...
                for parent in block.parents() {
                    if !dag.nodes.contains_key(parent) {
                        let stub = Block::new_pruned(vec![genesis_id], 0, 0, 0, *parent);
...
pub fn encode_block(block: &Block, buf: &mut Vec<u8>) {
    ...
    // RESERVED (was the VRF has_vrf flag). Always 0: VRF is removed. The byte
    // is retained so the encoding of a VRF-less block is byte-identical to
    // every pre-removal one.
    buf.push(0u8);
...
impl<'a> Reader<'a> {
    ...
    fn read_block_with_stored_id(&mut self, stored_id: BlockId) -> Result<Block, SnapshotError> {
        ...
        // VRF fields — REMOVED, but still skipped so that pre-removal
        // snapshots and logs remain readable. A block carrying VRF fields can
        // no longer be represented or verified, so the bytes are consumed and
        // discarded; the reconstructed block hashes identically because the
        // reserved flag byte it writes back is the same 0 these blocks had.
        //   v6+ : has_vrf, then if 1: pk(32), proof_flag(+96), output_flag(+32)
        //   v5  : has_vrf, then if 1: pk(32) + proof(96) + output(32)
        if self.version >= 5 {
            let has_vrf = self.read_u8()?;
            if has_vrf == 1 {
                let _pk = self.read_array::<32>()?;
                if self.version >= 6 {
                    if self.read_u8()? == 1 {
                        let _proof = self.read_array::<96>()?;
                    }
                    if self.read_u8()? == 1 {
                        let _output = self.read_array::<32>()?;
                    }
                } else {
                    let _proof = self.read_array::<96>()?;
                    let _output = self.read_array::<32>()?;
        ...
        if payload_len == 0 {
            ...
            // using the stored id.
            return Ok(Block::new_pruned_with_authority(
                parents,
                work,
                timestamp_ms,
                nonce,
        ...
        let block = if let Some(auth_sig) = authority_sig {

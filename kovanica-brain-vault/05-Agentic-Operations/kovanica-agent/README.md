...
```
docker-compose.yml       full stack wiring (docker.sock owned by sandbox-runner only)
docker-compose.cpu.yml   VPS / no-GPU override (Ollama, loopback :13080) — **required on hosts where port 8080 is occupied** (e.g., by kovanica-node). Always deploy with: `docker compose -f docker-compose.yml -f docker-compose.cpu.yml up -d`
deploy/                  nginx vhost + vps-up.sh

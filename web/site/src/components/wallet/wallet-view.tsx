...
export function WalletView() {
  ...
  const selectedAssetOpt = assetOptions.find(opt => 
  ...
  async function refreshChain(address: string) {
    try {
      const [u, h, f] = await Promise.all([
        ...
        api<{ fee_rate: number; unit: string; mempool: number; bytes: number }>("/api/fee_estimate", "GET"),
      ]);
      setUtxos(u);
      setHist(h);
        ...
        {isNftSelected && (
          <div className="rounded-lg border border-purple/30 bg-purple/5 p-3 flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted">Entire token (1 unit) will be sent. Cannot be split.</p>
            </div>
          </div>
        )}

        <label className="text-xs text-muted">
          Amount ({isNativeAsset(assetId) ? TOKEN : isNftSelected ? "NFT (fixed: 1)" : "asset units"})
